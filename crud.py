...
db_connection = mysql.connector.connect( database="crud_python", 
host="localhost",  
user="root",  
password="")  
...
class StudentApp(tk.Tk):
    def __init__(self):  
        super().__init__()  
        self.title("Administracion de estudiantes")  
        self.geometry("800x650")  
        self.lblTitle = tk.Label(self, text="Sistema de administracion de alumnos", font=("Helvetica", 16), bg="yellow", fg="green")  
        self.lblFName = tk.Label(self, text="Nombre:", font=("Helvetica", 10), bg="blue", fg="yellow")  
        self.lblLName = tk.Label(self, text="Apellido:", font=("Helvetica", 10), bg="blue", fg="yellow")  
        self.lblContactNo = tk.Label(self, text="Contacto:", font=("Helvetica", 10), bg="blue", fg="yellow")  
        self.lblCity = tk.Label(self, text="Ciudad:", font=("Helvetica", 10), bg="blue", fg="yellow")  
        self.lblState = tk.Label(self, text="Provincia:", font=("Helvetica", 10), bg="blue", fg="yellow")  
        self.lblDOB = tk.Label(self, text="Fecha de nacimiento:", font=("Helvetica", 10), bg="blue", fg="yellow")  
        self.lblSelect = tk.Label(self, text="Seleccione un dato a borrar o actualizar", font=("Helvetica", 10), bg="blue", fg="yellow")  
        self.lblSearch = tk.Label(self, text="Buscar numero de entrada:",font=("Helvetica", 10), bg="blue", fg="yellow")  
        ...
        self.btn_update = tk.Button(self,text="Actualizar",font=("Helvetica",11),bg="yellow", fg="blue",command=self.update_student_data)  
        self.btn_delete = tk.Button(self, text="Borrar", font=("Helvetica", 11), bg="yellow", fg="blue",  
        command=self.delete_student_data)  
        self.btn_clear = tk.Button(self, text="Limpiar", font=("Helvetica", 11), bg="yellow", fg="blue",  
        command=self.clear_form)  
        self.btn_show_all = tk.Button(self, text="Mostrar todos", font=("Helvetica", 11), bg="yellow", fg="blue",  
        command=self.load_student_data)  
        self.btn_search = tk.Button(self, text="Buscar", font=("Helvetica", 11), bg="yellow", fg="blue",  
        ...
        self.btn_show_all.place(x=630, y=245, height=25, width=100)  
    ...
    def exit(self):  
        MsgBox = mb.askquestion('Exit Application', '¿Desea salir de la aplicacion?', icon='warning')  
        if MsgBox == 'yes':  
            self.destroy()  
    ...
    def delete_student_data(self):
        try:
            ...
            mb.showinfo("Info", "Por favor seleccione una columna") 
            return
        MsgBox = mb.askquestion('Delete Record', '¿Estas seguro de borrar a este alumno de la tabla?', icon='warning')  
        if MsgBox == 'yes':  
            if db_connection.is_connected() == False:  
                db_connection.connect()  
            db_cursor.execute("use crud_python") # Interact with Student Database  
            # deleteing selected student record  
            Delete = "delete from student_master where RollNo='%s'" % (roll_no)  
            db_cursor.execute(Delete)  
            db_connection.commit()  
            mb.showinfo("Information", "Estudiante fue exitosamente eliminado")  
    ...
    def create_table(self):  
        ...
        db_cursor.execute("CREATE DATABASE IF NOT EXISTS crud_python") # Create a Database Named Student  
        db_cursor.execute("use crud_python") # Interact with Student Database  
    ...
    def register_student(self):  
        ...
        if fname == "":  
            mb.showinfo('Information', "Introduzca un nombre")  
            self.entFName.focus_set()  
            return  
        if lname == "":  
            mb.showinfo('Information', "Introduzca un apellido")  
            self.entLName.focus_set()  
            return  
        if contact_no == "":  
            mb.showinfo('Information', "Introduzca un numero de contacto")  
            self.entContact.focus_set()  
            return  
        if city == "":  
            mb.showinfo('Information', "Introduzca un nombre de ciudad")  
            self.entCity.focus_set()  
            return  
        if state == "":  
            mb.showinfo('Information', "Introduzca el nombre de la provincia")  
            self.entState.focus_set()  
            return  
        if dob == "":  
            mb.showinfo('Information', "Introduzca la fecha de nacimiento")  
    ...
    # Inserting record into student_master table of student database  
        ...
        try:  
            ...
            mb.showinfo('Information', "El estudiante fue registrado")  
            ...
            mb.showinfo('Information', "La insercion de los datos ha fallado")  
        finally:  
            db_connection.close()  
    ...
    def fetch_max_roll_no(self):  
        ...
        if db_cursor.rowcount == 0:  
            ...
            for row in rows:  
                rollno = row[0]  
                rollno = rollno + 1  
        print("Maximo Id de estudiante: " + str(rollno))  
        return rollno  
    ...
    def show_search_record(self):  
        ...
        # Inserting record into student_master table of student database  
        db_cursor.execute("use crud_python") # Interact with Bank Database  
        ...
        if total ==0:  
            mb.showinfo("Info", "No hay nada que mostrar, Porfavor agrege datos")  
            return  
        print("Total de entradas de dato:" + str(total))  
        rows = db_cursor.fetchall()  
        RollNo = ""  
        First_Name = ""  
        Last_Name = ""  
    ...
    def update_student_data(self):  
        ...
        try:
            ...
            mb.showinfo("Info", "Porfavor seleccione una columna") 
            return
        print("Actualizando")  
        db_cursor.execute("use crud_python") # Interact with Student Database  
        ...
        db_connection.commit()  
        mb.showinfo("Info", "El estudiante seleccionado fue actualizado con exito")  
        self.load_student_data() 
    ...
    def load_student_data(self):  
        ...
        if total ==0:  
            mb.showinfo("Info", "No hay nada que mostrar,Porfavor agrege datos")  
            return  
        print("Entradas de dato totales:" + str(total))  
        rows = db_cursor.fetchall()  
        RollNo = ""  
        First_Name = ""  
        Last_Name = ""  
